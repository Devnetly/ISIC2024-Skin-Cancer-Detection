...
from src.utils import load, seed_everything
from src.datasets import ISICDataset,ImagesDirectory
from src.models import ResNet,ViT
from src.trainer import Trainer
from pandas.errors import DtypeWarning
from torchmetrics.classification import BinaryAccuracy,BinaryF1Score
from src.preprocessing import ColorspaceConvertor,ReinhardAugmentation
import h5py
...
def create_datasets(config : Config) -> tuple[list[tuple[ISICDataset,ISICDataset]],ISICDataset]:

    train_transforms, val_transforms = create_transforms(config)

    metadata = pd.read_csv(os.path.join(ISIS_2024_DIR, 'metadata.csv'))
    hdf5_file = h5py.File(os.path.join(ISIS_2024_DIR, 'images.hdf5'), "r")
    ...
    for fold in range(5):
        ...
        if config.external_data:
            ...
            external_data = ImagesDirectory(
                ...
                img_transform=train_transforms,
                target_transform=T.Lambda(lambda x: 1.0),
                target_col='benign_malignant',
                image_col='isic_id'
            )

            train_dataset = ConcatDataset([train_dataset,external_data])
...
def create_dataloaders(config : Config) -> tuple[list[tuple[DataLoader,DataLoader]],DataLoader]:

    train_dataset, test_dataset = create_datasets(config)

    loaders = []

    for train_dataset,val_dataset in train_dataset:
        
        if not config.external_data:
            labels = np.array(train_dataset.get_labels())
        else:
            labels =np.array(train_dataset.datasets[0].get_labels() + [1] * len(train_dataset.datasets[1]))
...
def main(args: Args) -> None:
    ...
    train_val_loaders,_ = create_dataloaders(config)

    for fold,(train_loader,val_loader) in enumerate(train_val_loaders):
        ...
        trainer = Trainer() \
            .set_checkpoints_folder(fold_dir) \
        ...
        trainer.save(filename=f"checkpoint_fold={fold}.pt")
